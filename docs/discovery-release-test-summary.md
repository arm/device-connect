...
|---|---|---|
| Agent-tools focused regression gate | `subscribe`, `await_replies`, MCP bridge subscriptions, MCP capability event names, event-label subscription filtering | `44 passed in 1.05s` |
| Edge type/label focused gate | `EventDef` validation, label models, driver label extraction | `20 passed in 0.13s` |
| Edge predicate unit tests | Edge-side `where` dependency warning, bounded predicate evaluation, CEL compile/eval behavior | `21 passed in 0.28s` |
| Full NATS integration suite | D2D, D2O, lifecycle, conformance, hierarchical tools, selector discovery, invoke, broadcast, subscribe, large-fleet tests | `134 passed, 120 skipped in 157.90s` |
| Full Zenoh integration suite | D2D, D2O, lifecycle, conformance, hierarchical tools, selector discovery, invoke, broadcast, subscribe, streaming tests | `116 passed, 138 skipped in 236.31s` |
| Integration inventory | Hierarchical tools, selector discovery, invoke, broadcast, subscribe, large-fleet tests across NATS/Zenoh where applicable | `254 collected per backend-filtered run` |
...
| Subscriptions and replies | `subscribe("correlation:<id>")`, live top-level `event(...)`, label-filtered `event(name, key:value)`, snapshot `device(...).event(...)`, iterator protocol, race-safe buffer drain, `await_replies` partial/complete collection | `test_subscribe_correlation_drains_large_reply_stream`, `test_subscribe_top_level_event_selector_includes_late_joiners`, `test_subscribe_top_level_event_selector_keeps_event_label_filter`, `test_subscribe_device_event_selector_is_snapshot`, `test_subscribe.py` | High for correlation replies and event subscription semantics, including same-event-name devices with different event labels on NATS and Zenoh. |
...
| Event-label subscriptions | `subscribe("event(name, key:value)")` preserves event-label filters by subscribing only to matching device/event subjects; it does not fall back to name-only wildcard delivery. |
